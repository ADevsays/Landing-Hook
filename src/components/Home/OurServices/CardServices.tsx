...
function CardServices({title, img, characteristics}:Props) {
    return (
        <div 
            style={{
                backgroundColor: 'rgba(255,255,255,0.06)'
            }}
            className="flex items-center flex-col min-h-[380px] shadow-lg rounded-lg backdrop-filter backdrop-blur-lg w-[310px] p-5 select-none">
        <h4 className="min-h-[70px] flex-center text-lg font-bold text-center">
            ...
            <ul className="font-medium flex flex-col items-start w-full">
                {characteristics.map(characteristic=> (
                    <InfoCharacteristics info={characteristic}/>
                ))}
            </ul>
            <Button className="btn-primary p-0 w-full mt-3 py-2">Ver Planes</Button>
        </div>
    );
};
